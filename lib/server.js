...
module.exports = function server () {
  ...
  bot.on('message', (data) => {
    ...
    } else if (isMessage(data) && isBotSpecificCommand(data)) {
      const parameters: Array<String> = msg.text.split(" ");
      parameters.splice(1).forEach(repo => {
        getPullSpecificRequest(repo)
        .then(buildMessage)
        .then((message) => {
          bot.postMessage(data.channel, message, botParams)
        })
  ...
  function getPullSpecificRequest (repo) {
    console.log('Checking for pull requests...')

    return pullhub(repo, labels).catch((err) => { console.error(err) })
  }
